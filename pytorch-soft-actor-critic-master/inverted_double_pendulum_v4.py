...
class InvertedDoublePendulumEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(self, **kwargs):
        observation_space = Box(low=-np.inf, high=np.inf, shape=(55,), dtype=np.float64)
        MujocoEnv.__init__(
            self,
            frame_skip=5,
    ...
    def step(self, action):
        ...
        # print('==================================================================')
        tan = self.data.site_xmat[0][2] / self.data.site_xmat[0][0]
        angle_penalty = abs(math.atan(tan))
        ...
        # Terminal condition
        terminated = bool(abs(x) > 1) # True/False
        if self.render_mode == "human":
            self.render()

        return self.observ.copy()[0], r, terminated, False, {}
